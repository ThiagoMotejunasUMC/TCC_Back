...
def listar_produtos(
    ...
    busca: Optional[str] = None, categoria_id: Optional[int] = None,
    db: Session = Depends(get_db), _: Usuario = Depends(get_current_user)
):
    return crud_produto.listar_produtos(db, skip, limit, busca, categoria_id)
...
def criar_produto(
    ...
    return crud_produto.criar_produto(db, data, usuario_id=current_user.id)

@router.put("/{produto_id}", response_model=ProdutoResponse)
def atualizar_produto(
    produto_id: int, data: ProdutoUpdate,
    db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_user)
):
    return crud_produto.atualizar_produto(db, produto_id, data, usuario_id=current_user.id)
...
def deletar_produto(
    ...
    return crud_produto.deletar_produto(db, produto_id, usuario_id=current_user.id)