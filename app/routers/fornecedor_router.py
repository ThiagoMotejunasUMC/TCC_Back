...
def listar_fornecedores(
    skip: int = 0, limit: int = 1000, busca: Optional[str] = None,
...
def criar_fornecedor(
    ...
    return crud_fornecedor.criar_fornecedor(db, data, usuario_id=current_user.id)

@router.put("/{fornecedor_id}", response_model=FornecedorResponse)
def atualizar_fornecedor(
    fornecedor_id: int, data: FornecedorUpdate,
    db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_user)
):
    return crud_fornecedor.atualizar_fornecedor(db, fornecedor_id, data, usuario_id=current_user.id)
...
def deletar_fornecedor(
    ...
    return crud_fornecedor.deletar_fornecedor(db, fornecedor_id, usuario_id=current_user.id)